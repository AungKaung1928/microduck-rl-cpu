...
python test_model.py || exit 1

hr "2/4  what the MJCF contains"
python inspect_model.py --all || exit 1

hr "3/4  drop tests -- does the physics behave, does the shipped PD hold"
python drop_test.py --mode limp --z0 0.25 --seconds 5 || exit 1
python drop_test.py --mode hold --seconds 5 || exit 1
python drop_test.py --mode hold --variant walk --seconds 5 --no-render || exit 1
